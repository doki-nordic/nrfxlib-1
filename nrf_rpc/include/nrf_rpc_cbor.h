...
struct nrf_rpc_cbor_cmd_rsp_ctx {
	CborParser parser;
...
#define NRF_RPC_CBOR_CMD_ALLOC(_ctx, _group, _packet, _len, ...)	       \
	NRF_RPC_CMD_ALLOC(_ctx.base_ctx, _group, _ctx.packet, _len,	       \
			  __VA_ARGS__);					       \
	_packet = _nrf_rpc_cbor_encoder_init(&(_ctx), _len)
...
#define NRF_RPC_CBOR_EVT_ALLOC(_ctx, _group, _packet, _len, ...)	       \
	NRF_RPC_EVT_ALLOC(_ctx.base_ctx, _group, _ctx.packet, _len,	       \
			  __VA_ARGS__);					       \
	_packet = _nrf_rpc_cbor_encoder_init(&(_ctx), _len)
...
/* Internal function that initializes cbor encoder.
 */
